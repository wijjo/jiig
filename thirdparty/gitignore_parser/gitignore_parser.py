...
def prepare_ignore_patterns(patterns, base_dir, resolve_symlinks=False):
    ...
    for pattern in patterns:
        counter += 1
        rule = rule_from_pattern(pattern, base_path=Path(base_dir).resolve(),
                                 source=(base_dir, counter),
...
IGNORE_RULE_FIELDS = [
    ...
    'resolve_symlinks'  # True to resolve symlinks
]
    ...
    def match(self, abs_path):
        ...
        if self.base_path:
            if self.resolve_symlinks:
                ...
                rel_path = str(Path(os.path.abspath(abs_path)).relative_to(self.base_path))
        else:
