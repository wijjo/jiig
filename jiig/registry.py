...
from .util.log import abort, log_error, log_message
    ...
    def resolve(self,
        ...
        if isinstance(reference, str):
            try:
                ...
                error_function(f'Failed to import {self.name} module: {reference}',
                               exc,
        ...
        if registration is None:
            error_function(f'Failed to resolve registered'
                           f' {self.name}: {reference.__name__}')
            return None
        return registration
